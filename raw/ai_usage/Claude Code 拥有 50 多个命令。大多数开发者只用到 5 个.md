...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/Qpp3Lq1Xx3gsOQXYVIHgmSLTRXn66GV3KfSjxyOP52BsSewZciafcwUfAmrZKDP9cGdSC7xS2evGTicpy1nnrRQ3eoyQgAGE0wBWMj6yXPx6c/640?wx_fmt=png&from=appmsg)
...
```
# CLAUDE.md## Authentication- Use JWT tokens, not sessions- Store in httpOnly cookies## Testing- Write tests for all API endpoints- Use Jest, not Mocha## Error Handling- Return structured errors: { error: string, code: number }
```
...
```
/model sonnet          # 切换到 Sonnet 4.6 (日常利器)/model opus            # 切换到 Opus 4.6 (架构杀手)/model haiku           # 切换到 Haiku 4.5 (体力活专家)
```
...
```
Session cost: $2.47Input tokens: 48,392Output tokens: 12,847Model: claude-sonnet-4-20250514
```
...
```
/diff              # Show all changes/diff src/auth.ts  # Show changes to specific file
```
...
- **`claude --print "..."`**：一闪电战。执行单个查询后立即退出，非常适合脚本编写。
    ...
    ```
    result=$(claude --print "Generate a random UUID")echo $result
    ```
