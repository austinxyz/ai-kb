...
![图片](https://mmbiz.qpic.cn/mmbiz_png/vB15efNc2MiaO2B9QXicMDZJOMom8lUUMTspRHx1vesvDNpSfzcgu37OHumNv7FEYfSYCQo5G3kBd2ibaEWaCUJRRJw96xl6Ql8ibVGp7VViaCIM/640?wx_fmt=png&from=appmsg)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/vB15efNc2Miay3EnsgW1dHTkBicJesYWu4iaXzl23xzVZIz7ASYiaDB3HqQhBr3XYJDNFwaJZzF6ibDaKEEmuG7c5XI5H3KQiaItbkoDNwwgS3PR4/640?wx_fmt=png&from=appmsg)
...
```
# Quant Fundamental Research — US Equities## 技术栈- Python 3.11+ / Pandas / NumPy / Polars- 数据源：Bloomberg API, Compustat via WRDS, SEC EDGAR- 回测框架：自研engine（src/backtest/）- 因子库：src/factors/ | 策略库：src/strategies/## 代码规范- type hints + docstring- DataFrame列名snake_case- 因子命名：{category}_{name}_{window} 如 value_ep_ttm- 日期格式YYYY-MM-DD- 回测指标必含：Ann Return, Max DD, Sharpe, Turnover## 数据约定- Ticker格式：AAPL（纯ticker）或 AAPL US Equity（BBG）- GICS行业分类为默认行业体系- Portfolio权重之和 = 1.0- 缺失值：行业中位数填充优先- 收益率：对数收益率- 市值单位：百万美元
```
...
```
---paths:  - \"src/factors/**/*.py\"---# 因子开发规范- 因子函数接受DataFrame输入返回Series- 必须处理NaN- 必须包含IC测试
```
...
```
.claude/skills/factor-ic/├── SKILL.md           # 主指令├── template.md        # 报告模板├── examples/sample.md # 示例输出└── scripts/ic.py      # 分析脚本
```
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/vB15efNc2MgKjRoQd09wWaRN43rViaDE0SYqibv8nvx5t9MxUq0REPeaic3duMnEswAGJYF00pvEOLtPjCE8fdPR1iabKx5bpa6nwia4EheWxJYE/640?wx_fmt=png&from=appmsg)
...
**保护关键文件** — 生产策略配置（configs/prod_*）、密钥文件（.env）、回测引擎核心代码（engine.py），Claude碰不了。用PreToolUse + exit 2拦截。
